...
def test_nvenc_smoke_uses_valid_hevc_main10_geometry(monkeypatch):
    calls: list[list[str]] = []
    ...
    monkeypatch.setattr(gpu, "_cmd", fake_cmd)
    gpu._nvenc_smoke()
...
def test_command_failure_includes_process_output(monkeypatch):
    import subprocess

    def fail(*_args, **_kwargs):
        raise subprocess.CalledProcessError(234, ["ffmpeg"], output="NVENC diagnostic detail")
