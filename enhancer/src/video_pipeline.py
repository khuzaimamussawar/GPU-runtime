...
def _probe(path: Path) -> Probe:
    output = subprocess.check_output(["ffprobe", "-v", "error", "-show_streams", "-show_format", "-of", "json", str(path)], text=True)
    ...
    return Probe(width=int(video["width"]), height=int(video["height"]), fps=fps, duration=duration,
...
def _spatial(frame_bgr: np.ndarray, model_name: str, target_w: int, target_h: int, settings: dict[str, Any]) -> np.ndarray:
    ...
    if target_w <= w and target_h <= h: enhanced = cropped
        ...
        try: enhanced = try_upscale_bgr_trt(cropped, settings, target_w, target_h)
        except Exception as error:
            if is_fatal_cuda_error(error) or not settings.get("allowNativeFallback", True): raise
            print(f"[enhancer] TensorRT spatial fallback to native PyTorch: {error}", flush=True); enhanced = None
        if enhanced is None: enhanced = upscale_bgr(cropped, model_name, outscale=scale)
...
def _open_encoder(output: Path, width: int, height: int, fps: float, settings: dict[str, Any]) -> subprocess.Popen:
    args = ["ffmpeg", "-v", "error", "-f", "rawvideo", "-pix_fmt", "bgr24", "-s", f"{width}x{height}", "-r", f"{fps:.8f}", "-i", "pipe:0", "-an", *video_encoder_args(settings), "-movflags", "+faststart", "-y", str(output)]
    return subprocess.Popen(args, stdin=subprocess.PIPE)


def _mux_audio(video_only: Path, source: Path, output: Path, *, speed: float, timing_baked: bool, has_audio: bool) -> None:
    if not has_audio: video_only.replace(output); return
...
def run_fast_video(job: dict[str, Any], cancel_event, progress: Progress) -> dict[str, Any]:
    input_data = job.get("input") or {}; output_data = job.get("output") or {}; settings = job.get("settings") or {}
    source_url = str(input_data.get("url") or "").strip(); output_key = str(output_data.get("objectKey") or "").strip()
    if not source_url.startswith(("http://", "https://")) or not output_key: raise ValueError("video job requires input.url and output.objectKey")
    ...
    interpolation = str(settings.get("interpolationModel") or "none").lower(); target_fps = int(settings.get("targetFps") or 0)
    if target_fps and target_fps not in {30, 48, 60}: raise ValueError("targetFps must be 30, 48 or 60")
    timing_baked = bool(settings.get("smoothSlowMotion") or settings.get("timingBaked")); speed = float(settings.get("directorSpeed") or 1.0)
    if speed <= 0: raise ValueError("directorSpeed must be positive")
    if interpolation == "gimm-vfi-f": raise RuntimeError("GIMM_LICENSE_NOT_CLEARED")
    if interpolation not in {"none", "rife-4.9", "rife"}: raise ValueError(f"unsupported local interpolation model: {interpolation}")
    ...
    with tempfile.TemporaryDirectory(prefix="sb-enhancer-video-") as tmp:
        ...
        def emit(frame: np.ndarray) -> None:
            ...
            encoder.stdin.write(np.ascontiguousarray(frame, dtype=np.uint8).tobytes()); emitted += 1
        for current in frames:
            ...
            while next_output_t * effective_speed <= cur_pts + 1e-9:
                ...
                if alpha <= 1e-6: out = prev_sr
                elif alpha >= 1.0 - 1e-6: out = cur_sr
                elif cut or not neural_vfi: out = prev_sr if alpha < 0.5 else cur_sr
                else:
                    prev_rgb = cv2.cvtColor(prev_sr, cv2.COLOR_BGR2RGB); cur_rgb = cv2.cvtColor(cur_sr, cv2.COLOR_BGR2RGB)
                    try: out_rgb = try_interpolate_rife_trt(prev_rgb, cur_rgb, alpha, settings)
                    except Exception as error:
                        if is_fatal_cuda_error(error) or not settings.get("allowNativeFallback", True): raise
                        print(f"[enhancer] TensorRT RIFE fallback to native PyTorch: {error}", flush=True); out_rgb = None
                    if out_rgb is None: out_rgb = interpolate_rife(prev_rgb, cur_rgb, alpha)
                    out = cv2.cvtColor(out_rgb, cv2.COLOR_RGB2BGR)
                emit(out); next_output_t += 1.0 / fps_out
            decoded += 1; prev_raw, prev_pts, prev_sr = cur_raw, cur_pts, cur_sr
