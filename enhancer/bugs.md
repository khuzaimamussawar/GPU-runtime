...
- SceneBuilder2 PR #100 merged: coordinated repo-level Enhancer parity closeout.
- GPU-runtime PR #13 merged: shared runtime contract + parity CI coverage.
- SceneBuilder PR #97 was closed as superseded; its two-file `AMPERE_PLUS` provider-routing fix was cleanly carried onto current SceneBuilder `main` in PR #100 instead of merging stale ancestry.
- SceneBuilder PR #88 was closed as superseded because its SECURE-only RunPod change would regress the H3-style SECURE -> COMMUNITY fallback.

## Repo-level items closed
- Admin visibility/defaults: Enhancer Admin and H3 advanced text-encoder/Sage/Spectrum controls are allowlisted-admin only. Normal users use the safe H3 defaults. Enhancer normal-user output encoder remains NVENC CQ 17.
...
- FAST and QUALITY Dockerfiles verify FFmpeg exposes both `hevc_nvenc` and `libx265`; real NVENC hardware qualification is job-specific.
- `enhancer/runtime-contract.json` is mirrored in SceneBuilder and GPU-runtime for port 8000, H3 callback path, service kinds, 60s idle default, max-8 video pool, and encoder names/defaults/ranges. GPU parity CI watches and executes the contract test.
...
- The FAST image that logged `HTTP 200 NON_JSON` predates the merged GPU-runtime callback parity change because the final Docker layers were not rebuilt. Rebuild FAST/QUALITY before diagnosing callback-client behavior again.
...
1. Deploy SceneBuilder `main` and verify production D1 compatibility/bootstrap, especially engine idempotency partial index, dispatch locks, worker digest fields, config JSON, and pending-upscale columns.
2. Rebuild/push **FAST** and **QUALITY** Docker `latest` from merged GPU-runtime `main` (engine builder uses FAST).
3. Verify Docker Hub digest changed and new workers report the expected digest.
4. Smoke RunPod + Novita, Fresh Image ON/OFF, FAST ESRGAN, RIFE, QUALITY/FlashVSR, NVENC CQ, x265 CRF, engine resume, Force Rebuild, encoder-aware `Upscale remaining`, and cross-project warm-worker reuse.
5. Re-check callback logs only after rebuilt images are running; separately verify whether Cloudflare Access is allowing the machine-to-machine callback to reach the Worker.
