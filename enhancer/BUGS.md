...
- SceneBuilder2 PR #99: Enhancer UI/provider/control-plane fixes.
- GPU-runtime PR #12: Enhancer runtime/NVENC/x265/lifecycle fixes.
- Both must be green and diff-audited before merge. Do not touch H3 implementation files.
...
- Cloudflare Access has returned HTTP 200 sign-in HTML on `/api/projects/v2/h3/pod/events`; Enhancer correctly exposed this as `HTTP 200 NON_JSON` while H3 treats any 2xx as callback success.
...
- Enhancer schema bootstrap/compatibility must create/repair required tables, columns, indexes, dispatch locks, and config rows on Worker startup/request path even if a migration was missed.
...
## Merge / deploy / build checklist

1. Verify PR #99 and #12 heads are green.
2. Verify changed files contain no H3 implementation files.
3. Merge both PRs.
4. Deploy SceneBuilder Worker / confirm Cloudflare picked up merged `main`.
5. Rebuild and push Enhancer FAST + QUALITY `latest` images from merged GPU-runtime. Engine builder uses FAST.
6. Confirm Docker Hub digest changed and new pods report the new digest.
7. Smoke test RunPod and Novita separately.
8. Smoke test FAST ESRGAN, RIFE interpolation, QUALITY/FlashVSR, NVENC CQ, x265 CRF, engine resume without Force, and Force rebuild.
9. In the next work pass, implement/verify the admin-email visibility and H3-style cross-project warm-pod idle/lending behavior above before calling Enhancer fully complete.
